<?php namespace NaviOcean\Laravel;
...
class NameParser {
   ...
   */
  protected static function get_pro_suffix($name) {
    foreach (self::$dict['suffixes']['prof'] as $suffix) {
   ...
   */
  protected static function is_suffix($word, $name) {
    # Ignore periods, normalize case
    $word = str_replace('.', '', strtolower($word));
    # Search the array for our word
    $line_match = array_search($word, array_map('strtolower', self::$dict['suffixes']['line']));
    $prof_match = array_search($word, array_map('strtolower', self::$dict['suffixes']['prof']));
    # Break out for professional suffix matches first
    if ($prof_match !== false) {
      return self::$dict['suffixes']['prof'][$prof_match];
    }
    # Now test our edge cases based on lineage
    if ($line_match !== false) {
      # Store our match
      $matched_case = self::$dict['suffixes']['line'][$line_match];
      # Remove it from the array
      $temp_array = self::$dict['suffixes']['line'];
   ...
   * @return boolean
   */
  protected static function is_salutation($word) {
    $word = str_replace('.', '', strtolower($word));
    foreach (self::$dict['prefix'] as $replace => $originals) {
   ...
   * @return boolean
   */
  protected static function is_compound($word) {
    return array_search(strtolower($word), self::$dict['compound']);
  }
  /**
   ...
   * @return boolean
   */
  protected static function is_camel_case($word) {
  ...
  public static function safe_ucfirst($seperator, $word) {
